...
public class RecursiveMethods {
	...
	boolean isArithmeticArrayHelper(int i, int diff, int[] a) {
		 ...
		 */
		boolean result = true;
		if (i < a.length - 1 && (a[i + 1] - a[i] != diff)) {
			return false;
		} else if (i == a.length) {
			return result;
		}
	...
	boolean isArithmeticListHelper(int i, int diff, List<Integer> l) {
		 ...
		 */
		boolean result = true;
		if (i < l.size() - 1 && (l.get(i + 1) - l.get(i) != diff)) {
			return false;
		} else if (i == l.size()) {
			return result;
		}
	...
	int[] insert(int[] result, int[] a, int i, int index) {
		if (index != a.length) {
			result[index] = a[index];
			index = index + 1;
			insert(result, a, i, index);
		}
		result[a.length] = i;
		return result;
	}

	void sort(int[] result, int start, int end) {
		if (start < end) {
			sort(result, start + 1, end);
			if (result[start] < result[end]) {
				sort(result, start, end - 1);
	...
	void sort2(List<Integer> result, int start, int end) {
		if (start < end) {
			sort2(result, start + 1, end);

			if (result.get(start) < result.get(end)) {
				sort2(result, start, end - 1);
			} else if (result.get(start) > result.get(end)) {
	...
	public int[] mergeSortedArrays(int[] left, int[] right) {
		...
		mergeHelper(left, newRight, result, 0, 0, 0);
		return result;
	}

	void mergeHelper(int[] left, int[] right, int[] result, int m, int n, int k) {
	...
	int[] sortAscend(int[] newRight, int[] right, int i) {
		if (i != right.length) {
			newRight[i] = right[right.length - i - 1];
			i = i + 1;
			sortAscend(newRight, right, i);
		}
		return newRight;
	}
	...
	public List<Integer> mergeSortedLists(List<Integer> left, List<Integer> right) {
		...
		List<Integer> newRight = new ArrayList<Integer>(right.size());
		sortAscend2(newRight, right, 0);
		if (left.size() == 0) {
			return newRight;
		...
		mergeHelper2(left, newRight, result, 0, 0, 0, total);
		return result;
	}

	void mergeHelper2(List<Integer> left, List<Integer> right, List<Integer> result, int m, int n, int k, int total) {
		if (k == total - 1) {
			if (m == left.size()) {
	...
	List<Integer> sortAscend2(List<Integer> newRight, List<Integer> right, int i) {
		if (i != right.size()) {
			newRight.add(i, right.get(right.size() - i - 1));
			i = i + 1;
			sortAscend2(newRight, right, i);
		}
		return newRight;
	}
}
