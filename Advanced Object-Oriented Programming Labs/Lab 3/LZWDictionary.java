...
public class LZWDictionary {
	...
	public LZWDictionary(String characters) {
		...
		String[] items = characters.split("");
		List<String> parts = Arrays.asList(items);

		for (int i = 0; i < parts.size(); i++) {
			if (!this.dict.contains(parts.get(i))) {
				this.map.put(parts.get(i), counter);
				this.dict.add(counter, parts.get(i));
				counter++;
			}
		}
	}
	...
	public int indexOf(String s) {
		for (int i = 0; i < this.dict.size(); i++) {
			if (s.equals(this.dict.get(i)) && i >= 0) {
				return i;
			}
		}
		return -1;
	 ...
	 */
	public void reset() {
		this.dict.removeIf(s -> s.length() > 1);
		this.map.keySet().removeIf(s -> s.length() > 1);
	}
	...
	public String get(int index) {
		if (index < 0 && index >= this.dict.size()) {
	...
	public boolean hasIndex(int index) {
		if (index < this.dict.size() && index >= 0) {
			return true;
		}
		return false;
	}
	...
	public int add(String entry) {
		if (this.dict.contains(entry)) {
			return this.dict.indexOf(entry);
		}
		this.map.put(entry, this.dict.size());
		this.dict.add(this.dict.size(), entry);
		return this.dict.indexOf(entry);
	}
